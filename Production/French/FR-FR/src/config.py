...
#
# On importe les variables depuis le fichier JSON
#

# On vérifie la présence du fichier de configuration JSON
if os.path.isfile('config.json') :
    print('Fichier de configuration détecté')

if not os.path.isfile('config.json') :
    print('Fichier de configuration non détecté, création en cours')
    base_config = {
        'token': 'INSERT_TOKEN',
    ...
    with open('config.json', 'w') as file:
        ...
        print('Veuillez compléter le fichier de configuration avant de lancer le bot')
        quit()


# On importe les parametres du fichier JSON
